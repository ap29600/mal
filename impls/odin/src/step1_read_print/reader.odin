...
peek :: proc (r: ^Reader) -> (string, bool) {
  if r.match.start == -1 do return "", false
...
read_list :: proc (r: ^Reader) -> (res: []Ast, ok: bool) {
  ...
  if opening_token != matching[closing_token] { return {}, false }
  return result[:], true
}
...
read_atom :: proc (r: ^Reader) -> (res: Ast, ok: bool) {
  ...
  switch tok[0] {
    ...
    case '0'..'9': return strconv.parse_int(tok)
    case '-':
    if len(tok) > 1 && '0' <= tok[1] && tok[1] <= '9' { return strconv.parse_int(tok) }
    else { return Symbol(strings.clone(tok)), true }
